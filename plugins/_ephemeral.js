      ...
      case WA_MESSAGE_STUB_TYPE.CHANGE_EPHEMERAL_SETTING:
        if (chat.detect)
          this.sendMessage(chatUpdate.jid, (+m.messageStubParameters[0] ?
            'Pesan Sementara ON' :
            'Pesan Sementara OFF') + `\nButton / Tombol ${+m.messageStubParameters[0] ?
              'tidak akan' :
              'akan'} berfungsi`
            , 'extendedTextMessage')
        break
        ...
        switch (m.msg.type) {
          case 3:
            if (m.isGroup) {
              ...
              this.sendMessage(m.chat, ('*BUG GRUP TERDETEKSI, HAPUS CHAT INI BIAR GA EROR!!!*\n\n' + require('util').format(log)).padEnd(65536, '\n*BUG GRUP TERDETEKSI, HAPUS CHAT INI BIAR GA EROR!!!*'), 'extendedTextMessage')
              if (m.isGroup) this.groupRemove(m.chat, [m.sender])
