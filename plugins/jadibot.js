...
let handler = async (m, { conn, args, usedPrefix, command }) => {
  ...
  if ((args[0] && args[0] == 'plz') || global.conn.user.jid == conn.user.jid) {
    ...
    conn.readmore = global.conn.readmore + ''
    ...
    conn.connect().then(async ({ user }) => {
      ...
      if (!global.db.data.settings[user.jid].owner) parent.sendButton(m.chat, `Silahkan atur nomor Owner mu`, 'Ketik .setowner', 1, ['Set Owner', '.setowner'], 0)
      if (auth) return
    ...
    conn.on('close', () => {
      setTimeout(async () => {
        try {
          ...
          delete global.conns[i]
          global.conns.splice(i, 1)
          parent.sendMessage(global.owner[0] + '@s.whatsapp.net', `TEssss menghubunkan ulang`, MessageType.extendedText)
          if (obj) await conn.loadAuthInfo(obj)
          parent.sendMessage(conn.user.jid, `Menghubungkan...`, MessageType.extendedText)
          await conn.connect()
          parent.sendMessage(conn.user.jid, `Done`, MessageType.extendedText)
