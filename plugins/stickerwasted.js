...
let handler = async (m, { conn, command }) => {
  ...
  if (m.mentionedJid[0]) {
    let buffer = await (await fetch(await conn.getProfilePicture(m.mentionedJid[0]).catch(e => { throw `Orang yang kamu tag tidak ada PP atau PP nya sedang diprivasi` }))).buffer()
