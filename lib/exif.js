...
exports.modStick = (media, client, mek, from) => {
    ...
    try {
        ...
        spawn('webpmux', ['-set', 'exif', './sticker/data.exif', media, '-o', out])
            .on('exit', () => {
                client.sendMessage(from, fs.readFileSync(out), MessageType.sticker, { quoted: mek })
                fs.unlinkSync(out)
...
exports.modMedia = (media, client, mek, from, fps = 15) => {
    out = getRandom('.webp')
    try {
        ff(media)
            .on('error', (e) => {
            ...
            .on('end', () => {
                ...
                spawn('webpmux', ['-set', 'exif', './sticker/data.exif', out, '-o', _out])
                    .on('exit', () => {
                        client.sendMessage(from, fs.readFileSync(_out), MessageType.sticker, { quoted: mek })
                        fs.unlinkSync(out)
                        fs.unlinkSync(_out)
                        fs.unlinkSync(media)
                    })
