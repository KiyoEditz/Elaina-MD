...
/**
 * Menghapus user dari tabel users SQLite
 */
function deleteUser(db, jid) {
  if (db && jid) {
    try {
      return db.prepare('DELETE FROM users WHERE jid = ?').run(jid);
    } catch (e) {
      console.error(`[SQLite Error] Gagal menghapus user ${jid}:`, e.message);
    }
  }
}

/**
 * Menghapus chat dari tabel chats SQLite
 */
function deleteChat(db, jid) {
  if (db && jid) {
    try {
      return db.prepare('DELETE FROM chats WHERE jid = ?').run(jid);
    } catch (e) {
      console.error(`[SQLite Error] Gagal menghapus chat ${jid}:`, e.message);
    }
  }
}
