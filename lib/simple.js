...
exports.makeWASocket = (connectionOptions, options = {}) => {
    ...
    conn.getJid = (sender) => {
        ...
        if (typeof sender === 'object' && sender !== null) {
            sender = sender.id || sender.jid || sender.phoneNumber || sender.lid || ''
        }
        ...
        // Check if it's the bot's own LID
        if (conn.user) {
            let myLid = conn.user.lid ? conn.decodeJid(conn.user.lid) : null
            if (myLid && sender === myLid) {
                let myJid = conn.decodeJid(conn.user.id || conn.user.jid)
                conn.isLid.set(sender, myJid)
                return myJid
            }
        }

        // Check persistent LID mappings in database if available
        if (global.db?.data?.lid && global.db.data.lid[sender]) {
            const mapped = global.db.data.lid[sender]
            conn.isLid.set(sender, mapped)
            return mapped
        }

        // Search through conn.chats group metadata participants
        if (conn.chats) {
            for (const chat of Object.values(conn.chats)) {
                ...
                const user = participants.find((p) => {
                    const pLid = p?.lid ? conn.decodeJid(p.lid) : null
                    const pId = p?.id ? conn.decodeJid(p.id) : null
                    return (pLid && pLid === sender) || (pId && pId === sender)
                })
                if (user) {
                    let jid = user.phoneNumber || user.jid || user.id
                    if (jid) {
    ...
    */
    conn.sendFile = async (jid, path, filename = '', caption = '', quoted, ptt = false, options = {}) => {
        ...
        else if (/audio/.test(type.mime || options.mimetype)) (
        ...
        let message = {
            ...
            caption,
            ptt,
            mentions: await conn.parseMention(caption).catch(e => e),
            [mtype]: { url: pathFile },
            mimetype,
...
END:VCARD`.trim()
    ...
    */
    ...
    conn.sendButtonDoc = async (jid, content, footerText, button1, id1, quoted, options) => {
        ...
        const buttonMessage = {
            document: { url: "https://wa.me/6283196162908" },
            mimetype: global.doc,
        ...
        conn.sendMessage(jid, buttonMessage, { quoted, ephemeralExpiration: 86400, ...options })
    }



    conn.sendButton = async (jid, content, footer = '', amount = 1, list = [], quoted, options = {}) => {
        let buttons = [
            { buttonId: list[1] || 'say', buttonText: { displayText: list[0] || 'Say' }, type: 1 },
            { buttonId: list[3] || 'Hello', buttonText: { displayText: list[2] || 'Hello' }, type: 1 },
            { buttonId: list[5] || 'world', buttonText: { displayText: list[4] || 'World' }, type: 1 }
        ]
        let buttons_amount = amount == 1 ? buttons.splice(1, 2) : amount == 2 ? buttons.splice(2, 1) : buttons
        ...
        return await conn.sendMessage(jid, buttonMessage, { quoted, ephemeralExpiration: 86400, contextInfo: { mentionedJid: conn.parseMention(content + footer) }, ...options })
    }
     ...
     * @param {Object} options 
     */
    conn.sendButtonImg = async (jid, buffer, contentText, footerText, amount, list = [], quoted, options) => {
        let type = await conn.getFile(buffer)
        let { res, data: file } = type
        if (res && res.status !== 200 || file.length <= 65536) {
            try { throw { json: JSON.parse(file.toString()) } }
    ...
    */
    ...
    conn.parseMention = async (text = '') => {
        if (!text) return []
        const match = [...text.matchAll(/@([0-9]{5,16}|0)/g)].map(v => v[1])
        const out = []
        for (const id of match) {
            const lid = `${id}@lid`
            const jid = conn.getJid(lid)
            if (conn.isLid?.has(lid)) out.push(jid)
            else if (jid && jid !== lid) out.push(jid)
    ...
    conn.saveName = async (id, name = '') => {
        ...
        let metadata = {}
        if (isGroup) metadata = await conn.groupMetadata(id).catch(_ => ({})) || {}
    ...
    conn.scrapGame = async (api, path) => {
        ...
        do {
            ...
            json = await res.json()
            scrap = JSON.parse(fs.readFileSync(`./scrape/${path}.json`))
            await delay(2000)
        } while (scrap.includes(json))
        scrap.push(json)
...
exports.smsg = (conn, m, hasParent) => {
    ...
    if (m.key) {
        ...
        m.chat = conn.getJid(m.key.remoteJid || message.message?.senderKeyDistributionMessage?.groupId || '')
        m.isGroup = m.chat.endsWith('@g.us')
        m.sender = conn.getJid(m.key.fromMe && (conn.user.id || conn.user.jid) || m.participant || m.key.participant || m.chat || '')
        m.fromMe = m.key.fromMe || areJidsSameUser(m.sender, conn.user.id) || areJidsSameUser(m.sender, conn.user.jid)
    }
    if (m.message) {
        ...
        if (['ephemeralMessage', 'viewOnceMessage', 'viewOnceMessageV2', 'documentWithCaptionMessage'].includes(m.mtype) && m.msg?.message) {
            let nestedType = Object.keys(m.msg.message)[0]
            m.mtype = nestedType
            m.msg = m.msg.message[nestedType]
        }
        if (m.chat == 'status@broadcast' && ['protocolMessage', 'senderKeyDistributionMessage'].includes(m.mtype)) m.chat = (m.key.remoteJid !== 'status@broadcast' && m.key.remoteJid) || m.sender
        if (m.mtype == 'protocolMessage' && m.msg.key) {
            if (m.msg.key.remoteJid == 'status@broadcast') m.msg.key.remoteJid = m.chat
            if (!m.msg.key.participant || m.msg.key.participant == 'status_me') m.msg.key.participant = m.sender
            m.msg.key.fromMe = areJidsSameUser(conn.getJid(m.msg.key.participant), conn.user.id) || areJidsSameUser(conn.getJid(m.msg.key.participant), conn.user.jid)
            if (!m.msg.key.fromMe && areJidsSameUser(conn.getJid(m.msg.key.remoteJid), conn.user.id)) m.msg.key.remoteJid = m.sender
        }
        ...
        m.mentionedJid = (m.msg?.contextInfo?.mentionedJid?.length && m.msg.contextInfo.mentionedJid || []).map(jid => conn.getJid(jid))
        let quoted = m.quoted = m.msg?.contextInfo?.quotedMessage ? m.msg.contextInfo.quotedMessage : null
        if (m.quoted) {
            let type = Object.keys(m.quoted)[0]
            m.quoted = m.quoted[type]
            if (['ephemeralMessage', 'viewOnceMessage', 'viewOnceMessageV2', 'documentWithCaptionMessage'].includes(type) && m.quoted?.message) {
            ...
            m.quoted.chat = conn.getJid(m.msg.contextInfo.remoteJid || m.chat || m.sender)
            m.quoted.isBaileys = m.quoted.id && m.quoted.id.length === 22 || false
            m.quoted.sender = conn.getJid(m.msg.contextInfo.participant)
            m.quoted.fromMe = m.quoted.sender === conn.user.jid || areJidsSameUser(m.quoted.sender, conn.user.id)
            m.quoted.text = m.quoted.text || m.quoted.caption || m.quoted.contentText || ''
            m.quoted.name = conn.getName(m.quoted.sender)
            m.quoted.mentionedJid = (m.quoted.contextInfo?.mentionedJid?.length && m.quoted.contextInfo.mentionedJid || []).map(jid => conn.getJid(jid))
