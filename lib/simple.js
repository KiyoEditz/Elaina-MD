...
exports.makeWASocket = (connectionOptions, options = {}) => {
    ...
    conn.ev.on('group-participants.update', async function updateParticipantsToDb({ id, participants, action }) {
        ...
        let groupMetadata = Object.assign((conn.contacts[id].metadata || {}), await conn.groupMetadata(id).catch(_ => _))
    ...
    */
    conn.sendMedia = async (jid, path, quoted, options = {}) => {
        ...
        return await conn.sendMessage(jid, { [`${pase}`]: data, mimetype: mime, ...options }, { quoted, sendEphemeral: 'chat' })
    }
...
`.trim()
        ...
        }, { quoted, ...options, sendEphemeral: 'chat' })
    }
...
END:VCARD`.trim()
    ...
    */
    ...
    conn.fakeReply = (jid, text = '', fakeJid = conn.user.jid, fakeText = '', fakeGroupJid, options) => {
        return conn.sendMessage(jid, { text: text }, { sendEphemeral: 'chat', quoted: { key: { fromMe: fakeJid == conn.user.jid, participant: fakeJid, ...(fakeGroupJid ? { remoteJid: fakeGroupJid } : {}) }, message: { conversation: fakeText }, ...options } })
    }
    conn.reply1 = async (jid, text, quoted, men) => {
        return conn.sendMessage(jid, {
            text: text, jpegThumbnail: await (await fetch(thumbr1)).buffer(), mentions: men
        }, { quoted: quoted, sendEphemeral: 'chat' })
    }
    ...
    */
    ...
    conn.sendButtonDoc = async (jid, content, footerText, button1, id1, quoted, options) => {
        ...
        return await conn.sendMessage(jid, buttonMessage, { quoted, sendEphemeral: 'chat', forwardingScore: 99999, isForwarded: true, ...options })
    }
    ...
    conn.sendButton = async (jid, content, footer = '', amount = 1, list = [], quoted, options = {}) => {
        ...
        return await conn.sendMessage(jid, buttonMessage, { quoted, sendEphemeral: 'chat', ...options })
    }
    ...
    */
    ...
    conn.sendTemplate = async (jid, content, footerText, amount, list = [], quoted, options) => {
        const buttons = [
            { index: 1, urlButton: { displayText: dtu, url: urlnya } },
            { index: 2, callButton: { displayText: dtc, phoneNumber: phn } },
            { index: 3, quickReplyButton: { displayText: list[0], id: list[1] } },
            { index: 4, quickReplyButton: { displayText: list[2], id: list[3] } },
            { index: 5, quickReplyButton: { displayText: list[4], id: list[5] } }
        ]
        amount == 1 ? buttons.splice(0, 3) : amount == 2 ? buttons.splice(0, 4) : buttons
        const key = {
            text: content,
            footer: footerText,
            mentions: await conn.parseMention(content + footerText),
        ...
        return await conn.sendMessage(jid, key, { quoted, ...options })
    }
    ...
    conn.send3TemplateButtonImg = async (jid, buffer, content, footerText, button1, id1, button2, id2, button3, id3, quoted, options) => {
        ...
        conn.sendMessage(jid, buttonMessage, { quoted, sendEphemeral: 'chat', contextInfo: { forwardingScore: 99999, isForwarded: true }, ...options, sendEphemeral: 'chat' })
    }
    ...
    conn.sendTemplateButtonLoc = async (jid, buffer, contentText, footer, buttons1, row1, quoted, options) => {
        ...
        }), { userJid: conn.user.jid, quoted: quoted, sendEphemeral: 'chat', ...options });
    ...
    conn.sendTemplateButtonFakeImg = async (jid, buffer, content, footerText, btn1, id1, options) => {
        ...
        conn.sendMessage(jid, key, { sendEphemeral: 'chat', mentions: conn.parseMention(content + footerText), contextInfo: { forwardingScore: 99999, isForwarded: true }, ...options })
    }
    conn.send2TemplateButtonFakeImg = async (jid, buffer, content, footerText, btn1, id1, btn2, id2, quoted, options) => {
        ...
        conn.sendMessage(jid, key, { quoted, sendEphemeral: 'chat', contextInfo: { mentions: conn.parseMention(content + footerText), forwardingScore: 9999, isForwarded: true } })
    }
    ...
    */
    ...
    conn.saveName = async (id, name = '') => {
        ...
        let metadata = {}
        if (isGroup) metadata = await conn.groupMetadata(id).catch(_ => _)
    ...
    conn.getName = (jid = '', withoutContact = false) => {
        ...
        if (jid.endsWith('@g.us')) return new Promise(async (resolve) => {
            v = conn.chats[jid] || {}
            if (!(v.name || v.subject)) v = await conn.groupMetadata(jid).catch(e => e) || {}
