...
const fs = require('fs').promises; // Untuk membaca file JSON lokal
const qs = require('qs');
...
const chatbot = {
  getCharacters: async () => {
    try {
      ...
      const data = await fs.readFile('./lib/elaina.json', 'utf-8');
  ...
  chat: async (query, characterName) => {
    ...
    const config = {
      ...
      headers: {
        ...
        'referer': 'https://onlinechatbot.ai/chatbots/lila/',
